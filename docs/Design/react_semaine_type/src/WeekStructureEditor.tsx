...
import {
  BlockId, DayIndex, DAY_NAMES, DAY_SHORT, WeekStructure, WeekStructurePayload,
  canCreateBlock, createBlock, dissolveBlock, dutyDaysPerWeek, isContiguous, members,
  renameBlock, setNone, setSolo, toPayload, unitsPerWeek, warnings,
...
export function WeekStructureEditor({ value, onChange, title = 'La semaine type', readOnly = false }: WeekStructureEditorProps) {
  const [ref, width] = useElementWidth<HTMLElement>();
  const tier = tierOf(width);
  const [selection, setSelection] = useState<DayIndex[]>([]);

  useEffect(() => { if (readOnly) setSelection([]); }, [readOnly]);

  const sel = useMemo(() => [...selection].sort((a, b) => a - b), [selection]);
  const commit = (next: WeekStructure) => { setSelection([]); onChange(next, toPayload(next)); };
          ...
          return (
            ...
            >
              ...
              {tier === 'l'
                ...
                : <span className="wse__tag" aria-label={long}>{tag}</span>}
            ...
            {value.days.map((_, idx) => {
              ...
              return (
                <div key={i} className="wse__rail-cell">
                  {isMember && <span className="wse__rail-bar" data-first={i === first} data-last={i === last} />}
        ...
        <div className="wse__actions" data-active={sel.length > 0}>
          {sel.length === 0 ? (
            <span className="wse__hint">
              Touchez un ou plusieurs jours pour changer leur rôle. Un bloc peut réunir des jours non consécutifs — vendredi et dimanche sans le samedi, par exemple.
            </span>
          ) : (
            <>
              <div className="wse__sel">
                <div className="wse__sel-title">{plural(sel.length, 'jour sélectionné', 'jours sélectionnés')}</div>
                <div className="wse__sel-sub">{list(sel)}{sel.length === 1 ? ' · un bloc réunit au moins deux jours' : ''}</div>
              </div>
              <div className="wse__buttons">
                {/* Remplacer par le Button du design system (MUI en production). */}
                <button type="button" className="wse__btn wse__btn--primary" disabled={!canCreateBlock(value, sel)} onClick={() => commit(createBlock(value, sel))}>Créer un bloc</button>
                <button type="button" className="wse__btn" onClick={() => commit(setSolo(value, sel))}>Garde isolée</button>
                <button type="button" className="wse__btn" onClick={() => commit(setNone(value, sel))}>Pas de garde</button>
                <button type="button" className="wse__btn wse__btn--ghost" onClick={() => setSelection([])}>Annuler</button>
              </div>
            </>
          )}
        </div>
            ...
            return (
              <div key={b.id} className="wse__block" data-block={b.id}>
                ...
                <input
                  ...
                  disabled={readOnly}
                  onChange={e => { const next = renameBlock(value, b.id as BlockId, e.target.value); onChange(next, toPayload(next)); }}
                />
                <span className="wse__members">
                  {list(m)} · {m.length} jours{isContiguous(m) ? '' : ' · non consécutifs'}
                </span>
                {!readOnly && (
                  <button type="button" className="wse__dissolve" onClick={() => commit(dissolveBlock(value, b.id))}>Dissoudre</button>
                )}
              </div>
            );
          })}
